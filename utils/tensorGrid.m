...
%     disp('Generating Nodes')
   ...
   G.nodes = struct('num', numN, 'coords',  ([xCoord(:), yCoord(:), zCoord(:)]));
   clear xCoord yCoord zCoord 

   %--------------------------------------------------------------------------
   %% Generate neighbors -----------------------------------------------------
% disp('Generating Neighbours')
   % Cell index matrix
...
%                   [NZ1, NZ2] ];
   ...
   G.faces = struct('num',       numF,                   ...
                    ...
                    'neighbors', ([ [reshape(C( 1:sx+1, 2:sy+1, 2:sz+1), [], 1), reshape(C( 2:sx+2, 2:sy+1, 2:sz+1), [], 1)]; ...
                  [reshape(C( 2:sx+1, 1:sy+1, 2:sz+1), [], 1), reshape(C( 2:sx+1, 2:sy+2, 2:sz+1), [], 1)]; ...
                  [reshape(C( 2:sx+1, 2:sy+1, 1:sz+1), [], 1), reshape(C( 2:sx+1, 2:sy+1, 2:sz+2), [], 1)] ]),       ...
                    'tag',       zeros(numF, 1));
    clear C
                %-----------------------------------------------------------------------
   %% Generate face-edges ----------------------------------------------------
% disp('Generating Faces')

   % Node index matrix
   N = reshape(1 : numN, [sx+1, sy+1, sz+1]);
   G.faces.nodes = [reshape([reshape(N(1:sx+1, 1:sy  , 1:sz  ), 1, []); 
                             reshape(N(1:sx+1, 2:sy+1, 1:sz  ), 1, []); 
                             reshape(N(1:sx+1, 2:sy+1, 2:sz+1), 1, []); 
                             reshape(N(1:sx+1, 1:sy  , 2:sz+1), 1, [])], [], 1); ...
                    reshape([reshape(N(1:sx  , 1:sy+1, 1:sz  ), 1, []); 
                             reshape(N(1:sx  , 1:sy+1, 2:sz+1), 1, []); 
                             reshape(N(2:sx+1, 1:sy+1, 2:sz+1), 1, []); 
                             reshape(N(2:sx+1, 1:sy+1, 1:sz  ), 1, [])], [], 1); ...
                    reshape([reshape(N(1:sx  , 1:sy,   1:sz+1), 1, []); 
                             reshape(N(2:sx+1, 1:sy,   1:sz+1), 1, []); 
                             reshape(N(2:sx+1, 2:sy+1, 1:sz+1), 1, []); 
                             reshape(N(1:sx  , 2:sy+1, 1:sz+1), 1, [])], [], 1)];
   clear N
   %--------------------------------------------------------------------------
   ...
   foffset = 0;
   % Face index matrices
   FX = reshape(foffset + (1:numFX), sx+1, sy  , sz  ); 
   foffset = foffset + numFX;
   FY = reshape(foffset + (1:numFY), sx  , sy+1, sz  );
   foffset = foffset + numFY;
   FZ = reshape(foffset + (1:numFZ), sx  , sy  , sz+1);

  G.cells.faces = [reshape([reshape(FX(1:sx  , :, :), 1, []);
                            reshape(FX(2:sx+1, :, :), 1, []);
                            reshape(FY(:, 1:sy  , :), 1, []);
                            reshape(FY(:, 2:sy+1, :), 1, []);
                            reshape(FZ(:, :, 1:sz  ), 1, []);
                            reshape(FZ(:, :, 2:sz+1), 1, [])], [], 1), ...
 ...
 %% Generate cell nodes -------------------------------------------------
   if opt.cellnodes,
      ...
      dj = sx+1;
      dk = (sx+1)*(sy+1);
      G.cellNodes = [k, k+di, k+dj, k+di+dj, k+dk, k+di+dk, k+dj+dk, k+di+dj+dk];
   end
   %--------------------------------------------------------------------------
   %% Assemble structure -----------------------------------------------------
   G.cartDims = celldim;
